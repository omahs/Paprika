...
unsafe readonly struct Page
{
    ...
    public TPage As<TPage>() where TPage : unmanaged, IPage
    {
        TPage result = default;
        Unsafe.As<TPage, Page>(ref result) = this; // blunt copy over
        return result;
    }

    public void Clear() => new Span<byte>(_ptr, Size).Clear();

    public void CopyTo(in Page page) => new Span<byte>(_ptr, Size).CopyTo(new Span<byte>(page._ptr, Size));

    // gives 256 * 256 * 256 fan out
    private const int JumpLevelCount = 3;

    public bool TryGet(in NibblePath path, int depth, IInternalTransaction tx, out ReadOnlySpan<byte> value)
    {
        // poor man's polymorphism based on level
        if (depth < JumpLevelCount)
        {
            return As<JumpPage>().TryGet(path, depth, tx, out value);
        }

        return As<ValuePage>().TryGet(path, depth, tx, out value);
    }
    
    public void Set(in NibblePath path, in ReadOnlySpan<byte> value, int depth, IInternalTransaction tx)
    {
        // poor man's polymorphism based on level
        if (depth < JumpLevelCount)
        {
            return As<JumpPage>().Set(path, value, depth, tx);
        }

        return As<ValuePage>().Set(path, value, depth, tx);
    }
}

unsafe readonly struct MetadataPage : IPage
{
    [StructLayout(LayoutKind.Explicit, Size = Page.Size - PageHeader.Size, Pack = 1)]
    public struct Payload
    {
        [FieldOffset(0)] public int Root;

        [FieldOffset(Page.PageAddressSize)] public int NextFreePage;
    }
    ...
    public int GetNextFreePage() => Data.NextFreePage++;

    public void Abandon(int pageId)
    {
        // TODO: implement free page management
    }
}

unsafe readonly struct JumpPage : IPage
{
    public const int FanOut = Page.NibbleValues * Page.NibbleValues;
    public const int NibblesPerJump = 2;
    ...
    public bool TryGet(in NibblePath path, int depth, IInternalTransaction manager, out ReadOnlySpan<byte> value)
    {
        var addr = Data.Jumps[GetAddr(path)];
        ...
        return manager.GetAt(addr).TryGet(path.SliceFrom(NibblesPerJump), depth + 1, manager, out value);
    }

    public void Set(in NibblePath path, in ReadOnlySpan<byte> value, int depth, IInternalTransaction tx)
    {
        ref var addr = ref Data.Jumps[GetAddr(path)];
        Page page;
        
        if (addr == Page.Null)
        {
            page = tx.GetNewDirtyPage(out addr);
            page.Clear();
        }
    ...
    private static int GetAddr(NibblePath path) => path.GetAt(0) << Page.NibbleSize | path.GetAt(1);
}

[StructLayout(LayoutKind.Explicit, Size = Page.Size, Pack = 1)]
unsafe readonly struct ValuePage : IPage
{
    ...
    public const int LengthOfLenght = 1;
    public const int KeySlice = 1;
    public const ushort NullAddr = 0;
    ...
    public bool TryGet(in NibblePath path, int depth, IInternalTransaction tx, out ReadOnlySpan<byte> value)
    {
        var slice = path.SliceFrom(KeySlice);
        var index = path.GetAt(0);

        // encode key only once and then compare encoded
        Span<byte> rawKey = stackalloc byte[slice.HexEncodedLength];
        slice.WriteTo(rawKey);

        return TryGetImpl(index, in rawKey, out value, tx);
    }

    private bool TryGetImpl(int index, in Span<byte> rawKey, out ReadOnlySpan<byte> value, IInternalTransaction tx)
    {
        var addr = Data.Buckets[index];

        if (addr != NullAddr)
        {
            var pagePayload = new Span<byte>(_ptr, Page.Size);
            while (addr != NullAddr)
            {
                var search = pagePayload.Slice(addr);
                var leftover = search.Slice(rawKey.Length);
        ...
        // always check overflow if exists, this page might have been saturated before the current nibble was set
        if (Data.Next != Page.Null)
        {
            var next = tx.GetAt(Data.Next).As<ValuePage>();
            return next.TryGetImpl(index, rawKey, out value, tx);
        }
